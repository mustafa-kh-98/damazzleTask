  ...
  void didUpdateWidget(IndexedTransitionSwitcher oldWidget) {
    ...
    if (widget.index != oldWidget.index) {
      _ChildEntry newChild =
          _childEntries
              .where((entry) => entry.index == widget.index)
              .first;
      _ChildEntry oldChild =
          _childEntries
              .where((entry) => entry.index == oldWidget.index)
              .first;
      ...
      if (widget.reverse) {
        ...
        oldChild.secondaryController.forward().then((value) =>
            setState(() {
              oldChild.onStage = false;
              oldChild.primaryController.reset();
              oldChild.secondaryController.reset();
  ...
  Widget build(BuildContext context) =>
      ...
      Stack(
        ...
        children: _childEntries
            .map<Widget>((entry) =>
            TickerMode(
              enabled: entry.onStage,
              child: Visibility(
                key: entry.key,
                visible: entry.onStage,
...
class _ChildEntry {
  _ChildEntry({required this.index,
    required this.key,
